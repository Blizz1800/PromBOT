...
async def db_len(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_message.chat.id not in consts.ADMINS:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Usted no es *BOT_ADMIN*", parse_mode='Markdown')
        return
    len_c = DB['users'].estimated_document_count()
    await context.bot.send_message(chat_id=update.effective_chat.id, text=f'Usuarios del Bot: ```{len_c}```', parse_mode='Markdown')