...
async def extraer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if me['target']:
        ...
        await context.bot.send_message(chat_id=id, text=f"Es este, {me['target']}, su destinatario para la extracción? Si no es este, por favor introduzca su nuevo destinatario", reply_markup=kb                                              )
    else:
        kb = ReplyKeyboardMarkup([
            [ BTS['CANCEL'] ]
            ], resize_keyboard=True)
